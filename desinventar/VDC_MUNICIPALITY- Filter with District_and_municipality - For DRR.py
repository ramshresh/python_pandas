...
excel_file_drr_des = r"C:\learn2code\desinventar\DRR_Municipality_2_DES_Manually_matched.xlsx"

drr = pd.read_excel(excel_file_drr, "2018-2019")
des = pd.read_excel(excel_file_des, "localbodies")
drr_des_vdc = pd.read_excel(excel_file_drr_des, "manually_matched")
...
def drr2_get_dist_by_vdc(drr_vdc, drr_district):
    df_filtered = drr[
        (drr['VDC/Municipality'].str.contains(drr_vdc, na=False))
        &(drr['District'].notnull())
        ]
    vdc =df_filtered.iloc[0]['Localbody'] if (df_filtered.shape[0]>0 )else None
    if vdc is None:
        print ("None %s"%(vdc))
    else:
         print ("Not None but %s"%(df_filtered.iloc[0]['Localbody']))
    return vdc
...
for drr_vdc in drr_vdc_list:
    ...
    drr_district_uppercase = str(drr_vdc).replace('Metropolitan City','').replace('Submetropolitan City','').replace('Rural Municipality','').replace('Municipality','').replace('Submetropolitan City','').strip()
